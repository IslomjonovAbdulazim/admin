...
// Read-only content API for regular admins
export const adminContentApi = {
  // View Courses (Read-only)
  getCourses: async (): Promise<Course[]> => {
    const response = await api.get<Course[]>('/api/v1/content/courses')
    return response.data
  },

  // View Course Lessons (Read-only)
  getCourseLessons: async (courseId: number): Promise<Lesson[]> => {
    const response = await api.get<Lesson[]>(`/api/v1/content/courses/${courseId}/lessons`)
    return response.data
  },

  // View Lesson Words (Read-only)
  getLessonWords: async (lessonId: number): Promise<Word[]> => {
    const response = await api.get<Word[]>(`/api/v1/content/lessons/${lessonId}/words`)
    return response.data
  },
}

// Content Management API functions (Super Admin only)
